...
function OnClientUpdate(player, objectId)
    ...
    if thisHash ~= nil then
        ...
        machine_player_ip_address = string.format(
            "%i.%i.%i.%i", machine_player_first_ip_byte, machine_player_second_ip_byte, machine_player_third_ip_byte,
            machine_player_fourth_ip_byte
        ) -- Player's IP Address (127.0.0.1 if host)
...
function getplayer(player_number)
    player_number = tonumber(player_number)
        or raiseerror(
            "bad argument #1 to getplayer (valid player required, got " .. tostring(type(player_number)) .. ")"
        )
    -- player header setup
